...
namespace TTS.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Input.Role == TTSApplicationUserRole.Consultant)
                    {
                        var addToRolesResult = await _userManager.AddToRoleAsync(user, "Consultant");
                    ...
                    {
                        var addToRolesResult = await _userManager.AddToRoleAsync(user, "Client");
                        if (addToRolesResult.Succeeded)
                        {
        ...
        private TTSApplicationUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<TTSApplicationUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(TTSApplicationUser)}'. " +
                    $"Ensure that '{nameof(TTSApplicationUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<TTSApplicationUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<TTSApplicationUser>)_userStore;
        }
    }
}
